...
2026-09-11 更新：同步官方调价——新增方案 C（V4.1 Flash，2026-09-10 12:00 起，峰时窗口仅工作日）与方案 D（V4 Pro 自 2026-09-14 12:00 起按 Flash 价计费）；旧模型名 `deepseek-v4-flash` / `deepseek-v4-flash-vision-exp` 自方案 C 起按 Flash 价计费，并补齐 vision-exp 在方案 B 的单价。历史记录自动按生效时间切换，无需改配置。
...
- **计价状态**：高峰时段按东八区显示（09:00–12:00、14:00–18:00），判定也按东八区时钟；2026-09-10 起官方峰时仅限工作日，周末整天按闲时，界面会标注「仅工作日」。
...
价格为「缓存未命中 / 缓存命中 / 输出」，单位元/百万 tokens；闲时为高峰的一半。方案 C 起高峰时段仅周一至周五（东八区 09:00–12:00、14:00–18:00）。
