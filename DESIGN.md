...
官方页面（api-docs.deepseek.com/quick_start/pricing，2026-09-11 抓取；生效时间对照官方更新日志与公告）：
**方案 A（2026-08-16T16:00Z 之前）**：平价（元/百万 tokens）
- v4-flash：命中 0.02 / 未命中 1 / 输出 2；v4-pro：0.025 / 3 / 6
- USD：flash 0.0028/0.14/0.28；pro 0.003625/0.435/0.87
**方案 B（2026-08-16T16:00Z = 北京时间 2026-08-17 00:00 起）**：峰谷定价，高峰按东八区 09:00-12:00、14:00-18:00 每日判定（与官方公告一致），为闲时 2 倍
- v4-flash / v4-flash-vision-exp：闲时 命中0.05/未命中1.5/输出4.5，高峰 0.10/3.0/9.0（USD 0.007/0.22/0.66；0.014/0.44/1.32）
- v4-pro：闲时 0.15/4.5/13.5，高峰 0.30/9.0/27.0（USD 0.022/0.66/1.98；0.044/1.32/3.96）
- 旧模型 deepseek-chat/deepseek-reasoner 不在公告内，各方案均保持旧平价（chat 2/0.5/8 元，reasoner 4/1/16 元；USD 0.27/0.07/1.10、0.55/0.14/2.19）
**方案 C（2026-09-10T04:00Z = 北京时间 12:00 起，V4.1 Flash 降价）**：峰时窗口不变，但仅工作日计高峰（官方自本次起改为周一至周五；周末整天按闲时）
- deepseek-flash（V4.1 Flash）：高峰 命中0.04/未命中2/输出8，闲时 0.02/1/4（USD 0.006/0.30/1.20；0.003/0.15/0.60）
- 旧名 deepseek-v4-flash、deepseek-v4-flash-vision-exp 仍可调用但已路由到 V4.1 Flash，按 Flash 价计费
- v4-pro 价格不变（高峰 0.30/9.0/27.0）
**方案 D（2026-09-14T04:00Z = 北京时间 12:00 起）**：v4-pro 请求全部路由到 V4.1 Flash，按 Flash 价计费（持续到 V4.1 Pro 上线）

### 自动切换逻辑（涨价预适配）

`PriceScheme[]` 有序数组，每条 `effectiveFrom`（UTC 毫秒）。对每条 usage 记录取
`max(scheme.effectiveFrom <= record.time)` 的方案；方案带 `peak` 时按记录时间
在东八区（`peakOffsetMinutes = 480`）的小时判断高峰/闲时，带 `peakDays` 时再按同一时区的星期过滤（方案 C/D 仅周一至周五）。**将来再调价只需追加一条新方案，无需改代码。**
设置项 `priceMode`：`auto`（默认，按时间自动切换）/ 强制 A/B/C/D；另有 `customPrices`
...
补价后历史立刻回算。配置页状态区保留高峰时段（东八区）与币种；工作日限制的方案会标注「仅工作日」。
